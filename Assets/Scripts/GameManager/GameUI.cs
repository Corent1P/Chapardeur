...
public class GameUI : MonoBehaviour
{
    ...
    {
        if (gameManager != null) livesText.text = $"Lives: {gameManager.GetLives()}";
    }
    ...
    {
        if (gameManager != null) modeText.text = $"Mode: {gameManager.GetCurrentGameMode()}";
    }

    private void OnGameOver() => ShowResultScreen("GAME OVER", false);
    private void OnVictory() => ShowResultScreen("VICTORY!", true);
    ...
    private void ShowResultScreen(string title, bool success)
    {
        ...
        if (gameManager != null)
        {
            ...
            for (int i = 0; i < resultStars.Length; i++) resultStars[i].enabled = i < stars;
        }
    }

    private void OnPlayAgain() => UnityEngine.SceneManagement.SceneManager.LoadScene(
        UnityEngine.SceneManagement.SceneManager.GetActiveScene().name);

    private void OnMainMenu() => UnityEngine.SceneManagement.SceneManager.LoadScene("MainMenu");
